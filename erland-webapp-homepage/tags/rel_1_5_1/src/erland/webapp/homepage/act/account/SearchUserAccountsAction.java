...
public class SearchUserAccountsAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        EntityInterface[] entities = getEnvironment().getEntityStorageFactory().getStorage("homepage-useraccount").search(filter);
        ...
        try {
            for (int i = 0; i < accounts.length; i++) {
                ...
                if (useEnglish && StringUtil.asNull(account.getLogoEnglish()) != null) {
                    accountsPB[i].setLogo(account.getLogoEnglish());
                }
                if (useEnglish && StringUtil.asNull(account.getHeaderTextEnglish()) != null) {
                    accountsPB[i].setHeaderText(account.getHeaderTextEnglish());
                }
            }
