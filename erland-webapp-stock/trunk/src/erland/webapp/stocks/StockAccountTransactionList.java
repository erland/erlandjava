...
public class StockAccountTransactionList implements StockAccountTransactionListInterface {
    ...
    public boolean addTransaction(String broker, String stock, String date, double number, double value) {
        Date d = getDateFromString(date);
        if(d==null) {
            return false;
        }
        int pos = getStockEntryPosition(vector,broker,stock,d);
        if(pos<0) {
            vector.addElement(new StockAccountTransaction(broker,stock,d,number,value));
        }else {
            vector.set(pos,new StockAccountTransaction(broker,stock, d, number, value));
        }
        return true;
    }

    public boolean removeTransaction(String broker, String stock, String date) {
        Date d = getDateFromString(date);
        if(d==null) {
            return false;
        }
