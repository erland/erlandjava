...
<%
    ...
    if(account!=null) {
        ...
        while(it.hasNext()) {
            ...
            %>
            <tr><td nowrap><a class="bold-link" href="portal?do=stockaccountnewmultiple&value=&broker=<%=broker%>&addcmd=stockaccountaddmultiple"><%=brokerString%> : Lägg till (för värde)</a></td></tr>
            <%
        }
        %>
        </table>
        <table class="no-border">
        <tr><td nowrap><p class="bold">Mäklare</p></td><td nowrap><p class="bold">Aktie</p></td><td nowrap><p class="bold">Datum</p></td><td nowrap><p class="bold">Värde</p></td></tr>
        <%
