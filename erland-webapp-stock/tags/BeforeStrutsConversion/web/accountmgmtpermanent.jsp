...
<%
    ...
    if(account!=null) {
        ...
        while(it.hasNext()) {
            ...
            %>
            <tr><td nowrap><a class="bold-link" href="portal?do=stockaccountnewpermanent&number=&value=&broker=<%=broker%>&addcmd=stockaccountaddpermanent"><%=brokerString%> : Lägg till (antal aktier för värde)</a></td></tr>
            <tr><td nowrap><a class="bold-link" href="portal?do=stockaccountnewpermanent&number=&broker=<%=broker%>&addcmd=stockaccountaddpermanent"><%=brokerString%> : Lägg till (antal aktier)</a></td></tr>
            <%
        }
        %>
        </table>
        <table class="no-border">
        <tr><td nowrap><p class="bold">Mäklare</p></td><td nowrap><p class="bold">Aktie</p></td><td nowrap><p class="bold">Datum</p></td><td nowrap><p class="bold">Antal</p></td><td nowrap><p class="bold">Värde</p></td></tr>
        <%
        it = account.getPermanentEntries().iterator();
        while(it.hasNext()) {
