...
public class NewUserAccountAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (user != null) {
            saveErrors(request, Arrays.asList(new String[]{"datacollection.useraccount.new.user-exists"}));
            return;
        }
        if (!fb.getPassword1().equals(fb.getPassword2())) {
            saveErrors(request, Arrays.asList(new String[]{"datacollection.useraccount.new.passwords-dont-match"}));
            return;
        }
        ...
        UserAccount templateAccount = (UserAccount) getEnvironment().getEntityFactory().create("datacollection-useraccount");
        ...
        getEnvironment().getEntityStorageFactory().getStorage("datacollection-useraccount").store(templateAccount);
