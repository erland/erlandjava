1. LICENSE
==========
Copyright (C) 2003-2006 Erland Isaksson (erland_i@hotmail.com)
...
- datacollection.war (The web application)
...
- datacollection.sql
  Creates the tables needed in the database which will be accessed as jdbc/datacollection
- users.sql
...
- datacollection_upgrade_x_y.sql
  Creates the tables in the database which will be accessed as jdbc/datacollection from a
  database created previously in version x.y
...
jdbc/datacollection: Should have the tables defined in the file(database\datacollection.sql)
...
9. DEPLOY AND RUN
=================
You should now be able to deploy the datacollection.war file on you web-server and then access it
as for example:
http://mywebserver/datacollection
