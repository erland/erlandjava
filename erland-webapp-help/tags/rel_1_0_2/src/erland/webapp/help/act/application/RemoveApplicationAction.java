...
public class RemoveApplicationAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(application==null) {
            saveErrors(request, Arrays.asList(new String[]{"help.application.delete.application-dont-exist"}));
            return;
        }
        getEnvironment().getEntityStorageFactory().getStorage("help-application").delete(template);
        QueryFilter filter = new QueryFilter("allforapplication");
        filter.setAttribute("application",fb.getName());
        getEnvironment().getEntityStorageFactory().getStorage("help-applicationversion").delete(filter);
