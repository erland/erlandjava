...
public class NewUserAccountAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (user != null) {
            saveErrors(request, Arrays.asList(new String[]{"help.useraccount.new.user-exists"}));
            return;
        }
        if (!fb.getPassword1().equals(fb.getPassword2())) {
            saveErrors(request, Arrays.asList(new String[]{"help.useraccount.new.passwords-dont-match"}));
            return;
        }
        ...
        templateRole.setApplication("help");
        ...
        UserAccount templateAccount = (UserAccount) getEnvironment().getEntityFactory().create("help-useraccount");
        templateAccount.setUsername(fb.getUsername());
        getEnvironment().getEntityStorageFactory().getStorage("help-useraccount").store(templateAccount);
