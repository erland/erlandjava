...
public class RemoveApplicationVersionAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        ApplicationVersion version = (ApplicationVersion) getEnvironment().getEntityStorageFactory().getStorage("help-applicationversion").load(template);
        if(version==null) {
            saveErrors(request, Arrays.asList(new String[]{"help.application.version.delete.version-dont-exist"}));
            return;
        }
        getEnvironment().getEntityStorageFactory().getStorage("help-applicationversion").delete(template);
