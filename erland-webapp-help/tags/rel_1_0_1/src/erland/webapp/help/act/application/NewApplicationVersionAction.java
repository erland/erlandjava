...
public class NewApplicationVersionAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(entities.length>0) {
            fb.setPreviousVersion(((ApplicationVersion) entities[entities.length-1]).getVersion());
            fb.setOrderNo(((ApplicationVersion) entities[entities.length-1]).getOrderNo());
        }
    }
}