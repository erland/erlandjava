...
- issuetracking.war (The web application)
...
- issuetracking.sql:
  Creates the tables needed in the datbase which will be accessed as jdbc/issuetracking
- common.sql
...
jdbc/issuetracking: Should have the tables defined in the file(database\issuetracking.sql)
...
9. DEPLOY AND RUN
=================
You should now be able to deploy the issuetracking.war file on you web-server and then access it
as for example:
http://mywebserver/issuetracking
