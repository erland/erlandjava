...
public class NewUserAccountAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (user != null) {
            saveErrors(request, Arrays.asList(new String[]{"issuetracking.useraccount.new.user-exists"}));
            return;
        }
        if (!fb.getPassword1().equals(fb.getPassword2())) {
            saveErrors(request, Arrays.asList(new String[]{"issuetracking.useraccount.new.passwords-dont-match"}));
            return;
        }
        ...
        templateRole.setApplication("issuetracking");
        ...
        UserAccount templateAccount = (UserAccount) getEnvironment().getEntityFactory().create("issuetracking-useraccount");
        templateAccount.setUsername(fb.getUsername());
        templateAccount.setDescription(fb.getDescription());
        getEnvironment().getEntityStorageFactory().getStorage("issuetracking-useraccount").store(templateAccount);
