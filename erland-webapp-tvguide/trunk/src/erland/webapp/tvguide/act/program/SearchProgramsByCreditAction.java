...
public class SearchProgramsByCreditAction extends SearchProgramsAction {
    /** Logging instance */
    private static Log LOG = LogFactory.getLog(SearchProgramsByCreditAction.class);

    protected ProgramPB[] getPrograms(ActionMapping mapping, String username, SearchProgramFB fb, HttpServletRequest request) {
        return ProgramHelper.getAllProgramsAfterTime(getEnvironment(),username,fb.getDate(),fb.getDayOffset(),null,StringUtil.asNull(fb.getCredit()),mapping.findForward("add-subscription-link"),mapping.findForward("cover-link"),mapping.findForward("update-review-link"),mapping.findForward("searchbyname-link"),mapping.findForward("searchbycredit-link"));
    }
