...
public class NewUserAccountAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (user != null) {
            saveErrors(request, Arrays.asList(new String[]{"tvguide.useraccount.new.user-exists"}));
            return;
        }
        if (!fb.getPassword1().equals(fb.getPassword2())) {
            saveErrors(request, Arrays.asList(new String[]{"tvguide.useraccount.new.passwords-dont-match"}));
            return;
        }
        ...
        templateRole.setApplication("tvguide");
        ...
        UserAccount templateAccount = (UserAccount) getEnvironment().getEntityFactory().create("tvguide-useraccount");
        PropertyUtils.copyProperties(templateAccount,fb);
        getEnvironment().getEntityStorageFactory().getStorage("tvguide-useraccount").store(templateAccount);
