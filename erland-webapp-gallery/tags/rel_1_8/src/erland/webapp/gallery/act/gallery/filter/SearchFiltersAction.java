...
public class SearchFiltersAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        QueryFilter filter = new QueryFilter("all");
        EntityInterface[] entities = getEnvironment().getEntityStorageFactory().getStorage("gallery-filter").search(filter);
        FilterFB[] pb = new FilterFB[entities.length];
        for (int i = 0; i < entities.length; i++) {
            pb[i] = new FilterFB();
            PropertyUtils.copyProperties(pb[i], entities[i]);
        }
        request.setAttribute("filtersPB", pb);
    }
}