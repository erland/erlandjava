...
public class EditGalleryFilterAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        GalleryFilterFB fb = (GalleryFilterFB) form;
        GalleryFilter storage = (GalleryFilter) getEnvironment().getEntityFactory().create("gallery-galleryfilter");
        PropertyUtils.copyProperties(storage, fb);
        getEnvironment().getEntityStorageFactory().getStorage("gallery-galleryfilter").store(storage);
    }
}