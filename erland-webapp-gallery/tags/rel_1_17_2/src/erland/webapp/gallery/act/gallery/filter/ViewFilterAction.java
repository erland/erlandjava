...
public class ViewFilterAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        Filter template = (Filter) getEnvironment().getEntityFactory().create("gallery-filter");
        template.setId(fb.getId());
        Filter filter = (Filter) getEnvironment().getEntityStorageFactory().getStorage("gallery-filter").load(template);
