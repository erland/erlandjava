...
public class RemoveGalleryFilterAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        GalleryFilterFB fb = (GalleryFilterFB) form;
        GalleryFilter filter = (GalleryFilter) getEnvironment().getEntityFactory().create("gallery-galleryfilter");
        filter.setId(fb.getId());
        getEnvironment().getEntityStorageFactory().getStorage("gallery-galleryfilter").delete(filter);
    }
}