...
public class ViewSkinAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        SkinFB fb = (SkinFB) form;
        Skin template = (Skin) getEnvironment().getEntityFactory().create("gallery-skin");
        template.setId(fb.getId());
        Skin skin = (Skin) getEnvironment().getEntityStorageFactory().getStorage("gallery-skin").load(template);
        if (skin != null) {
            PropertyUtils.copyProperties(fb, skin);
        }
    }
}