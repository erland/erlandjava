...
public class RemoveMediaAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        getEnvironment().getEntityStorageFactory().getStorage("cdcollection-media").delete(template);
        QueryFilter filter = new QueryFilter("allformedia");
        filter.setAttribute("media", template.getId());
        getEnvironment().getEntityStorageFactory().getStorage("cdcollection-disc").delete(filter);
