...
2. LIBRARIES
============
This software also includes a library "Antelmann.com Java Framework" for
accessing cddb server, see http://www.antelmann.com for more information.
...
- cdcollection.war (The web application)
...
- cdcollection.sql
  Creates the tables needed in the database which will be accessed as jdbc/cdcollection
- users.sql
...
- cdcollection_upgrade_x_y.sql
  Creates the tables in the database which will be accessed as jdbc/cdcollection from a
  database created previously in version x.y
...
Data scripts:
- common_data.sql
  Setup coverart dir to "D:\users\erland\covers" (You probably want to change this)
  Setup coverart thumnail cache dir to "D:\users\erland\covers\thumbnails" (You probably want to change this)
  Setup default thumnail size to 80x60
...
jdbc/cdcollection: Should have the tables defined in the file(database\cdcollection.sql)
...
9. DEPLOY AND RUN
=================
You should now be able to deploy the cdcollection.war file on you web-server and then access it
as for example:
http://mywebserver/cdcollection
