...
public class ViewDiscTrackAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        DiscTrackFB fb = (DiscTrackFB) form;
        DiscTrack template = (DiscTrack) getEnvironment().getEntityFactory().create("cdcollection-disctrack");

        template.setId(fb.getId());
        DiscTrack disc = (DiscTrack) getEnvironment().getEntityStorageFactory().getStorage("cdcollection-disctrack").load(template);
        if (disc != null) {
