...
public class ViewPurchaseEntryAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        PurchaseEntryFB fb = (PurchaseEntryFB) form;
        PurchaseEntry template = (PurchaseEntry) getEnvironment().getEntityFactory().create("diary-purchaseentry");
        template.setId(fb.getId());
        template.setUsername(request.getRemoteUser());
        PurchaseEntry entry = (PurchaseEntry) getEnvironment().getEntityStorageFactory().getStorage("diary-purchaseentry").load(template);
