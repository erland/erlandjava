...
public class ViewDiaryCommand implements CommandInterface, ViewDiaryInterface {
    ...
    public String execute(HttpServletRequest request) {
        ...
        if(id!=null && id.length()>0) {
            Diary template = (Diary) environment.getEntityFactory().create("diary-diary");
            User user = (User) request.getSession().getAttribute("user");
            String username = user.getUsername();
            template.setId(Integer.valueOf(id));
            diary = (Diary) environment.getEntityStorageFactory().getStorage("diary-diary").load(template);
