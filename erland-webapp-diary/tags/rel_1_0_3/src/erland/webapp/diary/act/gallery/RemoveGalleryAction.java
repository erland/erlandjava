...
public class RemoveGalleryAction extends BaseAction {
    protected void executeLogic(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        Gallery gallery = (Gallery) getEnvironment().getEntityFactory().create("diary-gallery");
        gallery.setId(fb.getId());
        gallery = (Gallery) getEnvironment().getEntityStorageFactory().getStorage("diary-gallery").load(gallery);
        if (gallery.getUsername().equals(request.getRemoteUser())) {
            ...
            getEnvironment().getEntityStorageFactory().getStorage("diary-galleryentry").delete(filter);
            getEnvironment().getEntityStorageFactory().getStorage("diary-gallery").delete(gallery);
