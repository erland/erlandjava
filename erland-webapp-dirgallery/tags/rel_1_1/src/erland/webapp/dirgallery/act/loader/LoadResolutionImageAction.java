...
public class LoadResolutionImageAction extends LoadThumbnailAction {
    protected Integer getWidth(HttpServletRequest request, ThumbnailImageFB fb) {
        ...
        if (getGallery(request).getMaxWidth() != null && getGallery(request).getMaxWidth().intValue() > 0 && width != null && getGallery(request).getMaxWidth().compareTo(width) < 0) {
            ...
            if (getGallery(request).getDefaultResolution() != null && getGallery(request).getDefaultResolution().length() > 0) {
            ...
            } else if (getGallery(request).getMaxWidth() != null && getGallery(request).getMaxWidth().intValue() > 0) {
                width = getGallery(request).getMaxWidth();
            } else {
                width = new Integer(9999);
            }
        }
        return width;
    }
